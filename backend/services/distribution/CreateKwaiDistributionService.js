...
export async function createKwaiDistribution({
  userId,
  campaignId,
  mediaAssetId,
  caption,
  ...
  if (!cleanCaption) {
    throw requestError("Legenda do Kwai e obrigatoria.");
  }

  if (cleanCaption.length > 1800) {
    throw requestError("Legenda do Kwai excede o limite seguro interno.");
  }
  ...
  if (!connection) {
    throw requestError("Conexao ativa do Kwai nao encontrada.");
  }
  ...
  try {
    ...
    distribution.lastError = "Falha ao agendar publicacao no Kwai.";
