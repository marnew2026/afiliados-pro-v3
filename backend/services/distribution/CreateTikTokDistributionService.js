...
export async function createTikTokDistribution({
  userId,
  campaignId,
  mediaAssetId,
  caption,
  ...
  assertIntegrationOperational("tiktok", env);
  ...
  if (!campaign) throw requestError("Campanha nao encontrada ou inativa.", 404);
  ...
  if (!connection) throw requestError("Conexao ativa do TikTok nao encontrada.");
  ...
  try {
    ...
    distribution.lastError = "Falha ao agendar publicacao no TikTok.";
