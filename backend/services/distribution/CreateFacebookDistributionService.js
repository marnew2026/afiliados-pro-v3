...
export async function createFacebookDistribution({
  userId,
  campaignId,
  mediaAssetId,
  caption,
  ...
  assertIntegrationOperational("facebook", env);
  ...
  if (!cleanCaption) {
    throw requestError("Descricao do Reel e obrigatoria.");
  }

  if (cleanCaption.length > 4000) {
    throw requestError("Descricao do Reel excede o limite seguro.");
  }
  ...
  if (!connection) {
    throw requestError("Conexao ativa do Facebook nao encontrada.");
  }
  ...
  try {
    ...
    distribution.lastError = "Falha ao agendar publicacao no Facebook.";
