...
export async function listTikTokReviewOptions({
  userId,
  ...
  if (!cleanUserId) throw new Error("Usuario nao informado.");

  const [connection, campaigns, assets] = await Promise.all([
    connectionModel.findOne({ userId: cleanUserId, provider: "tiktok", active: true })
      .select("destinationName connectedAt")
      .lean(),
    campaignModel.find({ userId: cleanUserId, active: true })
      .select("nome")
      .sort({ createdAt: -1 })
      .lean(),
    mediaAssetModel.find({ userId: cleanUserId, type: "video", status: "ready" })
  ...
  const campaignNames = new Map(
    campaigns.map((campaign) => [String(campaign._id), String(campaign.nome || "Campanha")])
  );
  ...
  return {
    ...
    connection: connection ? {
      destinationName: String(connection.destinationName || "Conta TikTok"),
